...
function BridgePlayer(opts) {
    ...
    this.contract = new bridge.Contract();
...
BridgePlayer.prototype.onBid = function onBid(bid) {
    ...
    if (this.auction.isClosed()) {
        ...
        console.log('contract', this.contract.toString());
    } else if (this.auction.nextSeatToBid() === this.seat) {
        this.emit('make-bid', this);
    } else {
        this.send(`${this.seat.name} ready for ${this.auction.nextSeatToBid().name}'s bid`);
    }
};
...
BridgePlayer.prototype.connect = function connect(table) {
    ...
    this.table
        ...
        .on('deal', m => {
            ...
            this.auction.dealer = this.board.dealer;
